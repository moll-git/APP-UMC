<div class="flex flex-col md:flex-row h-full min-h-[calc(100vh-48px)]">
    ...
    <!-- Config Sidebar -->
    <div
        class="w-full md:w-[240px] md:min-w-[240px] bg-[#111111] border-b md:border-b-0 md:border-r border-[#222222] py-6 flex flex-col">
        <div class="px-4 pb-4">
        ...
        <div class="px-2 pb-2">
            <div class="flex items-center gap-2.5 p-2.5 rounded-lg bg-[#1a1a1a] border border-[#333333] mb-2">
                <div
                    class="w-[30px] h-[30px] rounded-full bg-[#444444] flex items-center justify-center text-xs font-semibold text-white shrink-0">
                    {{ strtoupper(substr(auth()->user()->name, 0, 1)) }}
                </div>
                <div class="truncate">
                    <div class="text-[13px] font-medium text-white truncate leading-tight">{{ auth()->user()->name }}
                    </div>
        ...
        <div class="px-2 flex-grow overflow-y-auto space-y-0.5">
            @php
                $sectionsList = [
                    [
                        'id' => 'cuenta',
                        'label' => __('app.settings_section_account'),
                        'desc' => __('app.settings_section_account_desc'),
                        'icon' =>
                            '<svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>',
                    ],
                    [
                        'id' => 'tema',
                        'label' => __('app.settings_section_theme'),
                        'desc' => __('app.settings_section_theme_desc'),
                        'icon' =>
                            '<svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><path d="M12 22C17.5228 22 22 17.5228 22 12C22 6.47715 17.5228 2 12 2C6.47715 2 2 6.47715 2 12C2 17.5228 6.47715 22 12 22Z"/><path d="M12 18A6 6 0 1 0 12 6A6 6 0 0 0 12 18Z"/></svg>',
                    ],
                    [
                        'id' => 'idioma',
                        'label' => __('app.settings_section_language'),
                        'desc' => __('app.settings_section_language_desc'),
                        'icon' =>
                            '<svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><line x1="2" x2="22" y1="12" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg>',
                    ],
                    [
                        'id' => 'notif',
                        'label' => __('app.settings_section_notif'),
                        'desc' => __('app.settings_section_notif_desc'),
                        'icon' =>
                            '<svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><path d="M6 8a6 6 0 0 1 12 0c0 7 3 9 3 9H3s3-2 3-9"/><path d="M10.3 21a1.94 1.94 0 0 0 3.4 0"/></svg>',
                    ],
                    [
                        'id' => 'ayuda',
                        'label' => __('app.settings_section_help'),
                        'desc' => __('app.settings_section_help_desc'),
                        'icon' =>
                            '<svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"/><line x1="12" x2="12.01" y1="17" y2="17"/></svg>',
                    ],
                    [
                        'id' => 'feedback',
                        'label' => __('app.settings_section_feedback'),
                        'desc' => __('app.settings_section_feedback_desc'),
                        'icon' =>
            ...
            @foreach ($sectionsList as $section)
                ...
                @endphp
                <div wire:click="changeSection('{{ $section['id'] }}')"
                    ...
                    <div class="min-w-0 flex-grow">
                        <div
                            class="text-[13px] {{ $isSecActive ? 'text-white font-medium' : 'text-[#888888]' }} leading-tight">
                            {{ $section['label'] }}
                        </div>
                    ...
                    @if (!$isSecActive)
                        <svg class="w-3 h-3 text-[#444444] shrink-0 ml-auto" fill="none" stroke="currentColor"
            ...
            <div class="max-w-[560px]">
                ...
                <div class="flex gap-5 items-start mb-8">
                    <div class="relative shrink-0">
                        <div
                            class="w-16 h-16 rounded-full bg-[#333333] border border-[#444444] flex items-center justify-center text-xl font-bold text-white">
                            {{ strtoupper(substr($name, 0, 1)) }}
                        </div>
                        <div
                            class="absolute -bottom-0.5 -right-0.5 w-5.5 h-5.5 rounded-full bg-white flex items-center justify-center cursor-pointer border border-[#0a0a0a]">
                            <svg class="w-3 h-3 text-black" fill="none" stroke="currentColor" stroke-width="2.5"
                ...
                <div class="space-y-5">
                    ...
                    <div>
                        ...
                        <input wire:model="instrument" type="text" readonly
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <div
                                class="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1 text-[11px] text-[#00aa55] bg-[#0d2a1a] px-2.5 py-0.5 border border-[#00aa5530] rounded-full">
                    ...
                    <div class="flex flex-wrap items-center gap-4 pt-2">
                        ...
                        <button
                            class="bg-transparent text-[#888888] hover:text-white text-sm py-2 px-3 underline decoration-dotted cursor-pointer transition select-none">
            ...
            <div class="max-w-[560px]">
                ...
                <div class="space-y-2.5">
                    @foreach ($themes as $theme)
                        ...
                        <div wire:click="$set('selectedTheme', '{{ $theme['id'] }}')"
                            ...
                            @if ($isThemeSelected)
                                <div
                                    class="w-[22px] h-[22px] rounded-full bg-[#4488ff] flex items-center justify-center shrink-0">
            ...
            <div class="max-w-[560px]">
                ...
                <div class="space-y-2">
                    @foreach ($languages as $lang)
                        ...
                        <div {{-- CAMBIO AQUÍ: Llamamos al método changeLanguage --}} wire:click="changeLanguage('{{ $lang['code'] }}')"
                    ...
                    {{ $isLangSelected ? 'bg-[#2a2a2a] border-[#444444]' : 'bg-[#1a1a1a] border-[#2a2a2a] hover:border-[#444444]' }}">
                            <span
                                class="text-xs tracking-wider font-bold select-none w-10 text-center bg-[#333333] py-1 rounded shrink-0">{{ $lang['flag'] }}</span>
                            <div class="flex-grow">
                                <div class="text-sm font-medium text-white">{{ $lang['name'] }}</div>
                            </div>
                            @if ($isLangSelected)
                                <svg class="w-4 h-4 text-[#00ff88]" fill="none" stroke="currentColor"
                                    stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"
                                    viewBox="0 0 24 24">
                                    <polyline points="20 6 9 17 4 12" />
                                </svg>
                    ...
                    @foreach ($notifications as $index => $n)
                        <div class="flex items-center gap-4 p-4 rounded-xl bg-[#1a1a1a] border border-[#2a2a2a]">
                            <div class="flex-grow min-w-0">
                                <div class="text-sm font-semibold text-white mb-0.5 truncate">{{ $n['label'] }}
                                </div>
                                <div class="text-xs text-[#666666] leading-relaxed">{{ $n['desc'] }}</div>
                            </div>
                            <!-- Custom Switch/Toggle -->
                            <button wire:click="toggleNotification({{ $index }})" type="button"
                                role="switch" aria-checked="{{ $n['enabled'] ? 'true' : 'false' }}"
                                ...
                                <span
                                    ...
                                    {{ $n['enabled'] ? 'translate-x-5' : 'translate-x-0' }}"></span>
                            </button>
                        </div>
                    @endforeach
                </div>
                ...
                <div class="space-y-2">
                    @foreach ($faqs as $i => $item)
                        ...
                        <div class="bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg overflow-hidden transition">
                            <div wire:click="toggleFaq('{{ $i }}')"
                                class="flex items-center justify-between p-4 cursor-pointer select-none">
                                <span
                                    class="text-sm font-semibold text-white leading-tight pr-4">{{ $item['q'] }}</span>
                                <svg class="w-4 h-4 text-[#555555] shrink-0 transition duration-150 {{ $isFaqOpen ? 'rotate-90 text-white' : '' }}"
                            ...
                            @if ($isFaqOpen)
                                <div
                                    class="px-4 pb-4 text-xs text-[#999999] leading-relaxed border-t border-[#2a2a2a] pt-3">
                                    {{ $item['a'] }}
                                </div>
                            @endif
                        </div>
            ...
            <div class="max-w-[560px]">
                ...
                @if ($feedbackSent)
                    <div class="p-5 bg-[#0d2a1a] border border-[#00aa5540] rounded-xl text-center">
                        ...
                        <div class="text-[15px] font-bold text-[#00ff88]">{{ __('app.settings_feedback_success') }}
                        </div>
                ...
                @else
                    <div class="space-y-5">
                        <div>
                            <label
                                class="block text-[11px] font-semibold tracking-wider text-[#888888] uppercase mb-2.5">
                            ...
                            <div class="flex gap-2.5 flex-wrap">
                                @foreach ([['bug', __('app.settings_feedback_bug')], ['mejora', __('app.settings_feedback_improve')], ['otro', __('app.settings_feedback_other')]] as $item)
                                    <button wire:click="$set('feedbackType', '{{ $item[0] }}')"
                        ...
                        <div>
                            <label
                                class="block text-[11px] font-semibold tracking-wider text-[#888888] uppercase mb-2">
                                {{ __('app.settings_feedback_desc_label') }}
                            </label>
                            <textarea wire:model.live="feedbackText" placeholder="{{ __('app.settings_feedback_placeholder') }}" rows="6"
                                class="w-full bg-[#1a1a1a] border border-[#333333] focus:border-[#555555] rounded-lg p-3 text-white text-sm outline-none resize-y transition"></textarea>
                        </div>

                        <button wire:click="sendFeedback" @if (empty(trim($feedbackText))) disabled @endif
